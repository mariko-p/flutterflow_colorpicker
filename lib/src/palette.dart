...
import 'package:flutterflow_colorpicker/src/utils.dart';
...
class _SliderLayout extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    positionChild(track, const Offset(12.0, 3.0));
    layoutChild(
      thumb,
      BoxConstraints.tightFor(width: 5.0, height: size.height / 2 + 1.0),
    );
    positionChild(thumb, Offset(0.0, size.height / 4 + 1.0));
    layoutChild(
      gestureContainer,
...
class TrackPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (trackType == TrackType.alpha) {
      ...
      Paint chessPaintB = Paint()..color = const Color(0xFF373737);
      Paint chessPaintW = Paint()..color = const Color(0xFF1B151F);
    ...
    switch (trackType) {
      ...
      case TrackType.alpha:
        ...
        Gradient gradient = LinearGradient(colors: colors);
        const border = BorderSide(color: Color(0xFF1E2428));
        paintBorder(
          canvas,
          rect,
          top: border,
